import PhyMLTask = require("../index");
...
jobManager.on("ready", () => {
    let a = new PhyMLTask.PhyMLTask({ "jobManager" : jmClient, "jobProfile" : "default" }, myOptions);
    a.on("processed", (a)=>{
        console.log(`OOOOO\n${util.inspect(a, {showHidden: false, depth: null})}`);
        
    });
    //a.pipe(process.stdout)
    fs.readFile("../data/gene186.aln", function (err, data) {
        if (err) throw err;
